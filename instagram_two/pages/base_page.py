...
class BasePage():
    ...
    def on_this_page(self, *args):
        for locator in args: self.get_element(locator)
    ...
    def get_text(self, locator, text):
        ...
        WebDriverWait(self.driver, 10).until(expected_condition, message="Unable to find expected text in the element")
        return self.get_element(locator).text

    def click_if_exists(self, locator):
        expected_condition = ec.presence_of_element_located(locator)
        WebDriverWait(self.driver, 10).until(expected_condition, message="Element does not exist")
